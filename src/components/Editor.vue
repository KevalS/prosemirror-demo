...
export default {
  ...
  mounted() {
    ...
    this.$on('_content-change-editor', (action) => {
      this.view.updateState(this.view.state.applyAction(action));
    ...
    setupProseMirror: function (content, editor) {
      ...
      this.view = new EditorView(editor, {
        ...
        onAction: (action) => {
          ...
          self.content.editor = this.view.state.doc;
          self.content.markdown = defaultMarkdownSerializer.serialize(this.view.state.doc);
        },
      });
...
document.addEventListener('mouseup', (e) => {
  ...
  for (var i = range.startOffset; i >= 0; i--) {
    if (range.startContainer.data[i]?.match(/\S/) != null) {
      startOffset++;
    } else break;
  }

  var endOffset = 0;
  for (var i = range.endOffset; i < range.endContainer.data.length; i++) {
    if (range.endContainer.data[i]?.match(/\S/)) {
      endOffset++;
    } else break;
  }

  startOffset = range.startOffset + 1 - startOffset;
